...
- [x] Dual timestamps: extract `ts_event` distinct from `ts_capture` during chunking. DoD: "meeting moved to Friday" captured Monday is retrievable by both dates. — Done: pure `EventTimeExtractor` (ISO/month-name dates, today/tomorrow/yesterday, weekday±next/this/last) resolves relative to CAPTURE time, wired into `Chunker.chunk` → fills `chunks.ts_event`. Precision-first (invalid dates + bare numbers → nil), earliest-wins, deterministic (injected TZ). 15 tests. NSDataDetector rejected (now-relative, non-deterministic). decisions.md logged. Time-of-day + multi-date-per-chunk deferred.

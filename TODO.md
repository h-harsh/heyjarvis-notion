...
- [ ] Event-driven AX-tree capture spike: AX notifications + NSWorkspace app-switch + typing-pause + **clipboard trigger + idle fallback**; no fixed-interval polling. DoD: frontmost-window text captured on change; a clipboard copy produces a `clipboard` event; idle runs zero capture cycles (CPU trace); replace verify check #4 with a fixture-driven capture assertion.

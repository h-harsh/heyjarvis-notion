...
### 2. Tests
Run: `swift test`
Observe: final line `Executed N tests, with 0 failures` (N ≥ 231 today) and exit 0.
...
- **Window-title normalization (spinner fix):** `WindowTitleNormalizer` strips animation glyphs (Braille block + circle/quadrant/Block-Elements throbbers) so a loading spinner's frames collapse to ONE stable title (`⠂ Build…`/`⠐ Build…` → same key — no episode churn); real titles with slashes/pipes/dashes are untouched; all-glyph/empty input → nil.
- **Walking skeleton (capture→store→search):** `CatalogStoreSink` drives episodes into the weekly-shard `ShardedCatalog` and the captured text is then found by `catalog.search` (provenance survives the round-trip); an empty episode isn't stored; the inner sink (JSONL spike) still receives the full stream; duplicate re-reads collapse yet stay searchable. This is the end-to-end DATA path (live TCC capture is the founder-machine half).
- **MCP recall-surface safety:** `MCPError` raw values are exactly `LOCKED`/`RATE_LIMITED`/`EMPTY_RANGE` (the §3a contract) with non-empty messages; `QueryThrottle` (anti-hammering, separate from key-unwrap rate limit) allows up to N queries per sliding window then blocks, the window slides open as served queries age out, and a rejected query does NOT consume capacity.
- **Append-only ledgers:** `egress_ledger`/`write_ledger`/`consent_log` DAOs append + read only (no update/delete) — appends accumulate immutably (5 appends → 5 rows, never coalesced), read back in ts-ascending order, a nullable `byte_count` round-trips as nil not 0, `write_ledger` records the full create→append→archive trail (drives undo) and filters by draft, and an egress row survives DB reopen unchanged.
- **Privileged Notion composer:** `NotionComposer.compose` turns a validated `FilingExtract` into valid Notion blocks (commitments→`to_do` with `(due …)`, log→`bulleted_list_item` under "Log", reading→under "Reading", empty→placeholder paragraph); the encoded payload contains NO `link` field, no `http`, no `](` — `NotionBlock` is a closed plain-text set that structurally cannot emit a URL/image, and the input type (`FilingExtract`, not raw text) enforces "never sees raw captured text."
